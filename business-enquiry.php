                ...
                <div class="row justify-content-lg-center card p-3">
                    <ul class="nav nav-tabs" id="myTab" role="tablist">
                        <li class="nav-item" role="presentation">
                            <button class="nav-link active" id="account-tab" data-bs-toggle="tab" data-bs-target="#account" type="button" role="tab" aria-controls="account" aria-selected="true">Business Enquiry</button>
                        </li>
                    </ul>
                    <div class="tab-content" id="myTabContent">
                        <div class="tab-pane fade show active" id="account" role="tabpanel" aria-labelledby="account-tab">                            
                            <div class="col-lg-12">
                                <br/><h1 class="page-header-title">Business Enquiry</h1><br/>
                                <table class="table table-bordered">
                                    ...
                                    <tbody id="catTable">
                                        ...
                                        $cu = $conn->link->query("SELECT * FROM `tb_business_enquiry` where be_ub_id=" . $cu_re['ub_id']);
                                        $cu_rec = $cu->fetchAll(PDO::FETCH_ASSOC);
                                        foreach ($cu_rec as $i => $cr) {
                                            ?>
                                            <tr>
                                                <td><?= $i + 1 ?></td>                                                
                                                <td><?= $cr['be_customer_name'] ?></td>
                                                <td><?= $cr['be_phone_number'] ?></td>
                                                <td><?= $cr['be_email'] ?></td>
                                                <td><?= $cr['be_added_on'] ?></td>                                                
                                            </tr>
                                        <?php } ?>
