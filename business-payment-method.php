...
<html lang="en">
    <head>
        ...
        <!-- Title -->
        <title>Business Payment Method</title>
                ...
                <div class="row justify-content-lg-center card p-3">
                    <ul class="nav nav-tabs" id="myTab" role="tablist">
                        <li class="nav-item" role="presentation">
                            <button class="nav-link active" id="account-tab" data-bs-toggle="tab" data-bs-target="#account" type="button" role="tab" aria-controls="account" aria-selected="true">View Business Payment Method</button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button class="nav-link" id="scurity-tab" data-bs-toggle="tab" data-bs-target="#scurity" type="button" role="tab" aria-controls="scurity" aria-selected="false">Add Business Payment Method</button>
                        </li>
                    </ul>
                    <div class="tab-content" id="myTabContent">
                        <div class="tab-pane fade show active" id="account" role="tabpanel" aria-labelledby="account-tab">                            
                            <div class="col-lg-12">
                                <br/><h1 class="page-header-title">Business Payment Method</h1><br/>
                                <table class="table table-bordered">
                                    <thead class="thead-light">
                                        <tr>                                    
                                            <th>Sr.No.</th>
                                            <th>Payment QR Image</th>
                                            <th>Payment Name</th>                                           
                                            <th>Action</th>
                                        </tr>
                                    </thead>
                                    <tbody id="catTable">
                                        ...
                                        $cu = $conn->link->query("SELECT * FROM `tb_business_payment` where bpa_ub_id=" . $cu_re['ub_id']);
                                        $cu_rec = $cu->fetchAll(PDO::FETCH_ASSOC);
                                        foreach ($cu_rec as $i => $cr) {
                                            ?>
                                            <tr>
                                                <td><?= $i + 1 ?></td>
                                                <td><img height="50" class="rounded-circle" src="./assets/uploads/<?= $cr['bpa_method_image'] ?>" alt="Image Description"></td>
                                                <td><?= $cr['bpa_method_name'] ?></td>
                                                <td>                                            
                                                    <a href="#?id=<?= $cr['bpa_id'] ?>&action=edit"><i class="bi bi-pencil-square"></i></a>
                                                    <a href="#?id=<?= $cr['bpa_id'] ?>&action=delete_buisness_details"><i class="bi bi-eye-fill"></i></a>
                                ...
                                <form method="post" action="./custom/rest/request.php" enctype="multipart/form-data">                                        
                                    ...
                                    <div class="row">                                             
                                        <div class="col-sm-6 mt-2">
                                            <label for="bp_name" class="form-label">Payment Method Name</label>
                                            <input type="text" class="form-control" id="bpa_method_name" name="bpa_method_name">                                            
                                        </div>
                                    </div>
                                    <div class="row">    
                                        <div class="col-sm-4 text-center">
                                            ...
                                            <button type="button" class="btn btn-primary btn-xs" onclick="document.getElementById('fileInput').click();">Upload Payment Method photo</button>
                                            <button type="button" class="btn btn-danger btn-xs" onclick="resetFileInput()">Reset</button>
                                        </div>
                                    </div>
                                    <div class="d-flex justify-content-end gap-3">                                
                                        <button type="submit" class="btn btn-primary btn-sm" name="business_payment_save" id="business_payment_save" value="business_payment_save">Save Changes</button>
                                        <button type="reset" class="btn btn-primary btn-sm"  name="business_payment_reset" id="business_payment_reset" value="business_payment_reset">Reset</button>
                                    </div>
                                </form>                                              
                            </div>
                        </div>
