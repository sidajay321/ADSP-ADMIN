                ...
                <div class="row justify-content-lg-center card p-3">
                    <ul class="nav nav-tabs" id="myTab" role="tablist">
                        <li class="nav-item" role="presentation">
                            <button class="nav-link active" id="account-tab" data-bs-toggle="tab" data-bs-target="#account" type="button" role="tab" aria-controls="account" aria-selected="true">View Business Certificate</button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button class="nav-link" id="scurity-tab" data-bs-toggle="tab" data-bs-target="#scurity" type="button" role="tab" aria-controls="scurity" aria-selected="false">Add Business Certificate</button>
                        </li>
                    </ul>
                    <div class="tab-content" id="myTabContent">
                        <div class="tab-pane fade show active" id="account" role="tabpanel" aria-labelledby="account-tab">                            
                            <div class="col-lg-12">
                                <br/><h1 class="page-header-title">Business Certificate</h1><br/>
                                <table class="table table-bordered">
                                    ...
                                    <tbody id="catTable">
                                        ...
                                        $cu = $conn->link->query("SELECT * FROM tb_business_certicate where bce_ub_id=" . $cu_re['ub_id']);
                                        $cu_rec = $cu->fetchAll(PDO::FETCH_ASSOC);
                                        foreach ($cu_rec as $i => $cr) {
                                            ?>
                                            <tr>
                                                <td><?= $i + 1 ?></td>
                                                <td><img height="50" class="rounded-circle" src="./assets/uploads/<?= $cr['bce_image'] ?>" alt="Image Description"></td>
                                                <td><?= $cr['bce_name'] ?></td>
                                                <td>                                            
                                                    <a href="#?id=<?= $cr['bce_id'] ?>&action=edit"><i class="bi bi-pencil-square"></i></a>
                                                    <a href="#?id=<?= $cr['bce_id'] ?>&action=delete_buisness_details"><i class="bi bi-eye-fill"></i></a>
                                ...
                                <form method="post" action="./custom/rest/request.php" enctype="multipart/form-data">                                        
                                    ...
                                    <div class="row">     
                                        <div class="col-sm-6 mt-2">
                                            <label for="ub_pan_number" class="form-label">Business PAN Number</label>
                                            <input type="text" maxlength="10" class="form-control" id="ub_pan_number" name="ub_pan_number">                                            
                                        </div>
                                        <div class="col-sm-6 mt-2">
                                            <label for="ub_gst_number" class="form-label">GST Number</label>
                                            <input type="text" maxlength="15" class="form-control" id="ub_gst_number" name="ub_gst_number">                                            
                                        </div>
                                        <div class="col-sm-6 mt-2">
                                            <label for="bce_name" class="form-label">Certificate Name</label>
                                            <input type="text" class="form-control" id="bce_name" name="bce_name">                                            
                                        </div>
                                    </div>
                                    <div class="row">    
                                        <div class="col-sm-4 text-center">
                                            ...
                                            <button type="button" class="btn btn-primary btn-xs" onclick="document.getElementById('fileInput').click();">Upload Certificate photo</button>
                                            <button type="button" class="btn btn-danger btn-xs" onclick="resetFileInput()">Reset</button>
                                        </div>
                                    </div>
                                    <div class="d-flex justify-content-end gap-3">                                
                                        <button type="submit" class="btn btn-primary btn-sm" name="business_certificatesave" id="business_certificatesave" value="business_certificatesave">Save Changes</button>
                                        <button type="reset" class="btn btn-primary btn-sm"  name="business_certificatereset" id="business_certificatereset" value="business_certificatereset">Reset</button>
                                    </div>
                                </form>                                              
                            </div>
                        </div>
